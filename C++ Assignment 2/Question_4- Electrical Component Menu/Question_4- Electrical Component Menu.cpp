#include <iostream>
using namespace std;

int main(){
	int choice;
	
	cout<<"===================================="<<endl;
	cout<<"Electrical Component Menu"<<endl;
	cout<<"===================================="<<endl;
	cout<<"1. Resistor"<<endl;
	cout<<"2. Capacitor"<<endl;
	cout<<"3. Diode"<<endl;
	cout<<"4. Transistor"<<endl;
	cout<<"5. LED"<<endl;
	
	cout <<"Enter your choice: ";
	cin>>choice;
	
	switch (choice){
		case 1:
			cout<<"Resistor: used to limit current flow in a circuit. "<<endl;
			break;
		case 2:
			cout<<"Capacitor: used to store electrical charges or energy. "<<endl;
			break;
		case 3:
			cout<<"Diode: Allows current to flow in one direction. "<<endl;
			break;
		case 4:
			cout<<"Transistor: Used to switch or amplifies signal. "<<endl;
			break;
		case 5:
			cout<<"LED: Emits light when current flows through it. "<<endl;
			break;
		default:
			cout<<"Invalid selection"<<endl;
			
	return 0;
	
	}
}
